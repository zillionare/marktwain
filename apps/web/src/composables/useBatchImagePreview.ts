...
async function replaceBlocksWithImageLinks(): Promise<boolean> {
  try {
    ...
    console.log(`原始内容行数:`, result.split(`\n`).length)
    console.log(`按 endLine 排序的图片块:`, sortedImages.map(img => ({
      id: img.id,
    ...
    for (const img of sortedImages) {
      if (img.startLine !== undefined && img.endLine !== undefined) {
        ...
        console.log(`处理图片 ${img.id}`)
        console.log(`  范围: 第${img.startLine}行到第${img.endLine}行`)
        console.log(`  替换前总行数: ${lines.length}`)
        ...
        console.log(`  替换后总行数: ${result.split(`\n`).length}`)
      }
    }

    console.log(`最终结果行数:`, result.split(`\n`).length)
