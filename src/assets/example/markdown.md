...
如果使用了不支持的 admonition 类型，系统会自动将其转换为 `note` 类型：

!!! info "信息提示"
    这个 `info` 类型不被支持，会自动转换为 `note` 类型。
    但标题会保持原来的 "信息提示"。

!!! success
    这个 `success` 类型也不被支持，会转换为 `note` 类型。
    标题会显示为 "Success"（首字母大写）。

!!! danger "危险警告"
    任何不支持的类型都会转换为 `note` 类型，确保内容能正常显示。
